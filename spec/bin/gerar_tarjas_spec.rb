...
describe "gerar_tarjas" do
  ...
  it "deve gerar o pdf corretamente" do
    ...
    comando = "gerar_tarjas.rb -m=1 -csv=#{File.dirname(__FILE__)}/../fixtures/teste.csv -pdf=#{File.dirname(__FILE__)}/../fixtures/tmp/teste_gerado.pdf"
    saida = %x[comando]
    hash_pdf_gerado = Digest::MD5.hexdigest(
      File.read("#{File.dirname(__FILE__)}/../fixtures/tmp/teste_gerado.pdf")
    )
